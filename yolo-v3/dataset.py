...
class DataGenerator():
    ...
    def generate(self):
        while True:
            X = np.zeros((self.batch_size, self.image_channel, self.image_size[0], self.image_size[1]))
...
if __name__ == "__main__":
    ...
    print(image_files[0])
    dataloader = DataGenerator(2, [512, 512], image_files, image_file_names, max_num = 1)
    ...
    for iter, (input, target) in enumerate(dataloader.generate()):
        if iter == 3:
            ...
            # Display the image
            ax.imshow(src)
            rect = patches.Rectangle((bb[0]-bb[2]/2,bb[1]-bb[3]/2),bb[2], bb[3], linewidth=3,edgecolor='r')
            ax.add_patch(rect)
