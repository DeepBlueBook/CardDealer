...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen, device):
    ...
    nA = num_anchors 
    nC = num_classes
    anchor_step = len(anchors)//num_anchors # sually anchors is pair of integer, so anchor_step is usually 2. 
    conf_mask  = torch.ones(nB, nA*nH*nW).to(device) * noobject_scale #confusion mask, if no obj exist,  5
    ...
    for b in range(nB):
        ...
        for t in range(50):  
            ...
            cur_gt_boxes = torch.FloatTensor([gx,gy,gw,gh]).repeat(nAnchors,1).to(device)
            #print(cur_gt_boxes)
            cur_ious = torch.max(cur_ious, bbox_iou(cur_pred_boxes, cur_gt_boxes ,xywh=True))
        #print("cur_ious",cur_ious.shape, "conf_mask", conf_mask.shape)
        conf_mask[b][cur_ious>sil_thresh] = 0
    if seen < 12800:
        if anchor_step == 4:
            tx = torch.FloatTensor(anchors).view(nA, anchor_step).index_select(1, torch.LongTensor([2])).view(1,nA,1,1).repeat(nB,1,nH,nW).to(device)
            ty = torch.FloatTensor(anchors).view(num_anchors, anchor_step).index_select(1, torch.LongTensor([2])).view(1,nA,1,1).repeat(nB,1,nH,nW).to(device)
        else:
            tx.fill_(0.5)
            ty.fill_(0.5)
        tw.zero_()
    ...
    for b in range(nB):
        for t in range(50):
            ...
            for n in range(nA):
                ...
                anchor_box = torch.FloatTensor([0,0, aw, ah]).view(1,4).to(device)
...
class V3Loss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        for i, stride in enumerate(self.strides):
            ...
            scale_split_output = output[:, self.output_split : self.output_split + nAnchors, 0:4] / self.reso  # Normarize 
            conf = output[:, self.output_split : self.output_split + nAnchors, 5]
            ...
            nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx, ty, tw, th, tconf,tcls = build_targets(vals["output"],vals["target"],vals["anchors"],vals["nA"],vals["nC"],vals["nH"],vals["nW"],vals["noobject_scale"],vals["object_scale"],vals["thresh"],vals["seen"],vals["device"])

            loss_x = self.coord_scale * nn.MSELoss(size_average=False)(scale_split_output[:,:,0]*coord_mask, tx*coord_mask)/2.0
            loss_y = self.coord_scale * nn.MSELoss(size_average=False)(scale_split_output[:,:,1]*coord_mask, ty*coord_mask)/2.0
            loss_w = self.coord_scale * nn.MSELoss(size_average=False)(scale_split_output[:,:,2]*coord_mask, tw*coord_mask)/2.0
            loss_h = self.coord_scale * nn.MSELoss(size_average=False)(scale_split_output[:,:,3]*coord_mask, th*coord_mask)/2.0
            loss_conf = nn.MSELoss(size_average=False)(conf*conf_mask, tconf*conf_mask)/2.0
            loss_cls = self.class_scale * nn.BCELoss(size_average=False)(cls, tcls)   # Todo
            loss = loss_x + loss_y + loss_w + loss_h + loss_conf + loss_cls
            print("loss_x {} loss_y {} loss_w {} loss_h {} loss_conf {} loss_cls {}".format(loss_x , loss_y ,loss_w , loss_h, loss_conf ,loss_cls))
...
if __name__=="__main__":
    ...
    parser.add_argument("--tiny", action="store_true", help="Using yolov3-tiny")
    ...
    if True:
        test_output = _output
        v3loss = V3Loss(num_classes=80, anchors=anchors, num_anchors=3, resolution=args.reso, strides=strides, device=device)
